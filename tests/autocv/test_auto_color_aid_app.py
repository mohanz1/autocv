...
def test_on_main_window_selected_ignores_empty_selection(app):
    app.main_window_picker = MagicMock()
    app.main_window_picker.get.return_value = ""
    app.controller.attach_main = MagicMock()

    app._on_main_window_selected(MagicMock())

    app.controller.attach_main.assert_not_called()


def test_refresh_main_windows_resets_picker_state(app):
    app.main_window_picker = MagicMock()
    app.child_window_picker = MagicMock()
    app.controller.window_titles = MagicMock(return_value=["Window 1"])
    app._has_valid_main = True
    app._has_valid_child = True

    app._refresh_main_windows()

    assert app._has_valid_main is False
    assert app._has_valid_child is False
    app.main_window_picker.__setitem__.assert_called_with("values", ["Window 1"])
    app.main_window_picker.set.assert_called_with("Select a main window")
...
def test_on_main_window_selected_handles_attach_failure(app):
    app.main_window_picker = MagicMock()
    app.child_window_picker = MagicMock()
    app.main_window_picker.get.return_value = "Main Window"
    app.controller.attach_main = MagicMock(return_value=False)
    app.controller.child_titles = MagicMock()

    app._on_main_window_selected(MagicMock())

    assert app._has_valid_main is False
    assert app._has_valid_child is False
    app.controller.child_titles.assert_not_called()
    app.child_window_picker.__setitem__.assert_called_with("values", [])
    app.child_window_picker.set.assert_called_with("No child windows found")


def test_on_child_window_selected_descends_until_no_more_matches(app):
    app.child_window_picker = MagicMock()
...
def test_on_child_window_selected_ignores_empty_selection(app):
    app.child_window_picker = MagicMock()
    app.child_window_picker.get.return_value = ""
    app.controller.attach_child = MagicMock()

    app._on_child_window_selected(MagicMock())

    app.controller.attach_child.assert_not_called()


def test_on_child_window_selected_handles_immediate_failure(app):
    app.child_window_picker = MagicMock()
    app.child_window_picker.get.return_value = "Child"
    app.controller.attach_child = MagicMock(return_value=False)

    app._on_child_window_selected(MagicMock())

    assert app.controller.attach_child.call_count == 1
    assert app._has_valid_child is False


def test_on_delete_selected_ignores_empty_selection(app):
    app.colors_listbox = MagicMock()
    app.colors_listbox.selection.return_value = []
    app._color_state = ColorSelectionState()

    app._on_delete_selected()

    app.colors_listbox.delete.assert_not_called()


def test_on_delete_selected_rebuilds_remaining_items(app):
    app.colors_listbox = MagicMock()
    app.colors_listbox.selection.return_value = ["b"]
    app.colors_listbox.get_children.return_value = ["a", "b", "c"]
    app._color_state = ColorSelectionState(pixels=[(1, 1, 10, 20, 30), (2, 2, 40, 50, 60), (3, 3, 70, 80, 90)])
    app._insert_listbox_item = MagicMock()
    app._update_best_color = MagicMock()

    app._on_delete_selected()

    assert app._color_state.pixels == [(1, 1, 10, 20, 30), (3, 3, 70, 80, 90)]
    app.colors_listbox.delete.assert_called_once_with("a", "b", "c")
    assert app._insert_listbox_item.call_count == 2
    app._update_best_color.assert_called_once()


def test_update_best_color_syncs_cached_state(app):
    app.info_label = MagicMock()
    app._color_state = ColorSelectionState(pixels=[(0, 0, 10, 20, 30)], best_color=(10, 20, 30), best_tolerance=1)

    app._update_best_color()

    assert app._best_color == (10, 20, 30)
    assert app._best_tolerance == 1
    app.info_label.config.assert_called_once()
...
def test_get_mouse_coords_in_frame_rejects_invalid_frame_and_zero_display(app):
    app._has_valid_main = True
    app._has_valid_child = True
    app.controller = MagicMock()
    app.controller.frame = np.zeros((20, 20), dtype=np.uint8)
    app._photo_image = MagicMock()
    app._photo_image.width.return_value = 10
    app._photo_image.height.return_value = 10

    assert app._get_mouse_coords_in_frame(MagicMock(x=1, y=1)) == (None, None)

    app.controller.frame = np.zeros((20, 20, 3), dtype=np.uint8)
    app._photo_image.width.return_value = 0
    app._photo_image.height.return_value = 10
...
def test_show_3x3_region_handles_empty_and_out_of_bounds(app):
    ...
    app.pixel_region_label.config.assert_called_with(image="", text=_NO_REGION_TEXT)

    app.pixel_region_label.config.reset_mock()
    app._has_valid_main = True
    app._has_valid_child = True
    ...
    app._show_3x3_region()
    app.pixel_region_label.config.assert_called_with(image=app._pixel_region_photoimage, text="")
...
def test_draw_color_markers_marks_best_and_all_samples(app):
    ...
    app.controller.autocv.find_color.assert_called_once_with((10, 20, 30), tolerance=5)
...
def test_update_image_covers_placeholder_and_capture_branches(app):
    ...
    assert app._photo_image is None
    ...
    app.controller.frame = np.zeros((2, 2), dtype=np.uint8)
    app._update_image()
    app.image_label.config.assert_called_with(text=_NO_CAPTURE_TEXT, image="")
    assert app._photo_image is None
...
def test_mouse_event_handlers_delegate_and_update_state(app):
    ...
    app._show_3x3_region.reset_mock()
    app._get_mouse_coords_in_frame.return_value = (None, None)
    app._on_mouse_move(MagicMock(x=0, y=0))
    app._show_3x3_region.assert_not_called()

    app._insert_listbox_item = MagicMock()
    app._update_best_color = MagicMock()
    app.controller = MagicMock()
    app.controller.frame = np.zeros((3, 3, 3), dtype=np.uint8)
    app._get_mouse_coords_in_frame.return_value = (1, 2)
    app._on_mouse_click(MagicMock(x=0, y=0))
    app._insert_listbox_item.assert_called_once()
    app._update_best_color.assert_called_once()

    app._insert_listbox_item.reset_mock()
    app._update_best_color.reset_mock()
    app._get_mouse_coords_in_frame.return_value = (None, None)
    app._on_mouse_click(MagicMock(x=0, y=0))
    app._insert_listbox_item.assert_not_called()
    app._update_best_color.assert_not_called()
